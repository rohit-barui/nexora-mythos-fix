...
class RiskScorer:
    """
    Risk Score Formula:
    RiskScore = (CVSS * 0.30) + (EPSS * 10 * 0.25) + (KEV_Bonus * 0.20)
                + (Criticality * 0.15) + (Exposure * 0.10)
    ...
    def calculate_risk_score(
        ...
        exposure_multiplier = {"internet-facing": 10.0, "internal": 5.0, "isolated": 1.0}.get(
            exposure_level.lower(), 5.0
        )
